document.addEventListener('DOMContentLoaded', function () {
  ...
  inputId.addEventListener('input', function () {
    if (inputId.value.trim() !== '') {
      ...
      fetch('http://localhost:8080/api/directions/get/' + inputId.value)
        .then(response => {
          if (!response.ok) {
        ...
        .then(direccion => {
          ...
          document.getElementById("clienteId").value = direccion.clienteId.id;
        })
        .catch(error => {
  ...
  formulario.addEventListener('submit', function (event) {
    ...
    var url = "http://localhost:8080/api/directions/update/" + direccion.id;
    fetch(url, {
      method: 'PUT',
      headers: {
